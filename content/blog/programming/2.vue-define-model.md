...
updatedAt: ""
title: "【Vue.js 3.4】defineModelを使ったカスタムインプットを作ろう"
...
## defineModelを使ったカスタムインプットの作成

インプット属性が存在するDOMをコンポーネント化する場合、今までは属性を定義するための`props`と、親コンポーネントへ値を受け渡すための`emits`、どちらも記載する必要がありました。

今回追加された`defineModel`を使うと`props`と`emits`を記述することなく、カスタムインプットを実現することができます。
